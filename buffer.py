...
# Utilities for safe copy/cast when storing rollout data.
...
class A2CBuffer(_BaseBuffer):
    # Lower-level rollout buffer (multi-intersection transitions).
    ...
    def compute_returns(
        ...
        # Per-tls discounted return / advantage over the collected window.
        T = len(self)
        if T == 0:
    ...
    def get(self) -> Dict[str, Any]:
        # Flatten [time, tls] to sample-level batch for shared-parameter update.
        if len(self) == 0:
...
class PPOBuffer(_BaseBuffer):
    # Upper-level PPO rollout buffer (one item per upper decision step).
    ...
    def compute_returns(
        ...
        # Supports both GAE and plain discounted-return targets.
        T = len(self)
        if T == 0:
